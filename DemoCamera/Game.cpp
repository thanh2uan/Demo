...
bool Game::Initialize(HWND hWnd, int width, int height)
{
	...
	camera = new Camera(width, height, D3DXToRadian(0), DirectX::XMFLOAT3(1.0f, 1.0f, 1.0f));

	background = new GameSprite(-400, -350, 0);
	if (!background->Initialize(gDevice->device, "space.png", 2880, 1280))
	{
		return false;
	}

	player = new GameSprite(0, 0, 200);
	if (!player->Initialize(gDevice->device, "spaceship.png", 72, 43))
	{
		return false;
	}
